...
public class Client {
  ...
  private AtomicBoolean running = new AtomicBoolean(true); // if client runs，客户端是否正在运行
  ...
  private class Connection extends Thread {
    private InetSocketAddress server;             // server ip:port，IPC服务器的地址和端口
    ...
    private Hashtable<Integer, Call> calls = new Hashtable<Integer, Call>();//当前正在处理的远程过程调用
    private AtomicLong lastActivity = new AtomicLong();// last I/O activity time，IPC连接最后一次通信时间
    private AtomicBoolean shouldCloseConnection = new AtomicBoolean();  // indicate if the connection is closed，连接关闭标记
    ...
    /**该方法是将一个IPC调用放入IPC连接中，如果连接的成员变量shouldCloseConnection为true，addCall()会返回false，
     * 也就防止将一个IPC调用放入一个正处于关闭过程中的连接。IPC连接可以再多个地方触发，进入关闭过程，但直到
     * Connection.close()方法中，ConnectionId和连接的映射关系才会被删除。连接被删除后getConnection()才有机会构
     * 造出一个新的连接<br/>
    ...
    private synchronized void setupConnection() throws IOException {
      ...
      while (true) {
        try {
          this.socket = socketFactory.createSocket();//创建Socket对象
          this.socket.setTcpNoDelay(tcpNoDelay);//设置不使用Nagle算法
   ...
   * pool.  Connections to a given ConnectionId are reused. */
    ...
    * 为了提高通信效率，客户端会复用到服务器的连接，并通过连接标识ConnectionId区分不同的连接<br/>
    * 客户端通过<remoteAddress, protocol, ticket>三个成员变量来区分连接
    */
   static class ConnectionId {
     ...
     static ConnectionId getConnectionId(InetSocketAddress addr,
       ...
       /**remotePrincipal变量表示是使用Kerberos验证还是简单的验证方式**/
