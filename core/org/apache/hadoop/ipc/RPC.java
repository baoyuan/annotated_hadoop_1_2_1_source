...
import org.hadoopinternal.ipc.IPCQueryStatusImpl;
...
public class RPC {
   ...
   * SocketFactory创建套接字时使用的套接字工厂，套接字工厂可以用于创建应用了不同策略的套接字，如带有安全性或支持防火墙隧道的套接字*/
  ...
  /** Construct a client-side proxy object that implements the named protocol,
   * talking to a server at the named address. 
   * @see #getProxy(Class, long, InetSocketAddress, Configuration)
   * @param factory 创建套接字时使用的套接字工厂，套接字工厂可以用于创建应用了不同策略的套接字，如带有安全性或支持防火墙隧道的套接字
  ...
  /** Construct a client-side proxy object that implements the named protocol,
   * talking to a server at the named address. 
   * @see #getProxy(Class, long, InetSocketAddress, Configuration, SocketFactory, int)
  ...
  /** Construct a client-side proxy object that implements the named protocol,
   * talking to a server at the named address. 
   * @see #getProxy(Class, long, InetSocketAddress, UserGroupInformation, Configuration, SocketFactory, int)
   * @param connectionRetryPolicy 如果方法执行失败，指定一种重试策略，这个参数如果为null，那么会在Client.ConnectionId.getConnectionId()方法中创建
   * @param checkVersion 是否检查服务器端接口版本
   * @return 返回一个客户端的代理对象
   * */
  public static VersionedProtocol getProxy(
      Class<? extends VersionedProtocol> protocol,
    ...
    //构造一个实现了protocol接口的Java动态代理对象
    VersionedProtocol proxy = (VersionedProtocol)Proxy.newProxyInstance(
        protocol.getClassLoader(), new Class[]{protocol}, invoker);
    
    if (checkVersion) {
      checkVersion(protocol, clientVersion, proxy);//调用getProtocolVersion()
    }
    return proxy;
  }

  /** Get server version and then compare it with client version. 
   * 检查服务器端的接口版本，protocol.getName()表示需要接口的规范名，为什么要这个参数呢?以HDFS的NameNode为例，由于
   * NameNode实现了多个IPC接口，所以必须根据客户端发过来的不同接口确定返回的版本号。
   * */
  public static void checkVersion(Class<? extends VersionedProtocol> protocol,
	  ...
	  //检查服务器端的接口版本
	  //protocol.getName()表示需要接口的规范名，为什么要这个参数呢?以HDFS的NameNode为例，由于
	  //NameNode实现了多个IPC接口，所以必须根据客户端发过来的不同接口确定返回的版本号。
   ...
   * 
   * @param protocol IPC接口对象，可以通过IPC接口的静态成员class直接获得。接口的静态成员class保存了该接口的java.lang.Class实例,
   * 		java.lang.Class类的实例表示正在运行的Java应用程序中的类和接口，提供来一系列与Java反射相关的重要功能
   * @param clientVersion 第二个参数是接口版本，接口会根据需求不断地进行升级，形成多个版本的IPC接口，如果客户端和服务器端使用的IPC接口版本不一致，结果
			 将是灾难性的，所以再建立IPC时，需要对IPC的双方进行版本检查
   * @param addr 服务器的Socket地址，用于建立IPC的底层TCP连接
   * @param conf Configuration对象，用于定制IPC客户端参数
   * @return a proxy instance 代理实例对象
   * @throws IOException
   */
  public static VersionedProtocol getProxy(
      Class<? extends VersionedProtocol> protocol,
  ...
  public static class Server extends org.apache.hadoop.ipc.Server {
    ...
    public static void main(String[] args) {
    	...
    	String serverName = classNameBase(queryService.getClass().getName());
