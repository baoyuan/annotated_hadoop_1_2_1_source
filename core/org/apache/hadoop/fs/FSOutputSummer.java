 ...
 * data before it is written to the underlying stream<br/>
 * 提供在流上计算校验和并输出的能力
 */
...
abstract public class FSOutputSummer extends OutputStream {
  ...
  private int write1(byte b[], int off, int len) throws IOException {
    ...
    //将数据读入对象的缓冲区
    int bytesToCopy = buf.length-count;//buf还能保持多少数据
    bytesToCopy = (len<bytesToCopy) ? len : bytesToCopy;//读入数据
    sum.update(b, off, bytesToCopy);//读入数据计算校验和
    System.arraycopy(b, off, buf, count, bytesToCopy);
    count += bytesToCopy;
    if (count == buf.length) {
      // local buffer is full
      flushBuffer();//缓冲区满，可以输出
    } 
    return bytesToCopy;
  }
  ...
  throws IOException {
    ...
    writeChunk(b, off, len, checksum);//由子类实现，如ChecksumFSOutputSummer
  }
