...
public class RawLocalFileSystem extends FileSystem {
  ...
  /** Convert a path to a File.<br/>
   * 将Path对象转换成File对象 
   * */
  public File pathToFile(Path path) {
    checkPath(path);
    if (!path.isAbsolute()) {
      path = new Path(getWorkingDirectory(), path);//如果是相对路径，则转换为绝对路径
    }
  ...
  /**
   *  继承自FileInputStream，改写了所有的read方法，以提供文件读取字节数的统计功能
   *
   */
  ...
  /**
   * 在目录上调用listStatus方法，可以获得一个目录下所有项目（包括文件和子目录）的元信息
   * 在文件上调用可以获得文件的元信息。
   */
   ...
   * treat existence as an error.<br/>
   * 
   */
  public boolean mkdirs(Path f) throws IOException {
    Path parent = f.getParent();
    File p2f = pathToFile(f);
    //创建目录，如果parent==null，则f即为根目录，否则mkdirs(parent)执行，mkdirs是一个递归调用。
    //&&后面的表达式也是一个||表达式，p2f.mkdir()创建目录如果创建失败，则执行p2f.isDirectory()
    //如果p2f是一个目录，则表示该目录存在，返回true，否则就表示存在p2f这个文件，那么就返回false
